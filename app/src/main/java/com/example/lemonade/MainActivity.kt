...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        when(lemonadeState) {
            SELECT -> {
                ...
                val three: LemonTree = lemonTree
                lemonSize = three.pick()
            ...
            SQUEEZE -> {
                /**
                 * When the image is clicked in the SQUEEZE state the squeezeCount needs to be INCREASED by 1
                 * and lemonSize needs to be DECREASED by 1.
                 */
                squeezeCount += 1
                lemonSize -= 1
        ...
        // lastly, before the function terminates we need to set the view elements so that the UI can reflect the correct state
        setViewElements()
    }
    ...
    private fun setViewElements() {
        ...
        when(lemonadeState){
            // fr each state, the textAction TextView should be set to the corresponding string from
